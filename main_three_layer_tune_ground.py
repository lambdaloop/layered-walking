...
N = len(bout['angles']['L1'])

all_ground_pos = []
for leg in legs:
    get_pos = ground.get_positions[leg]
    b = bout['angles'][leg]
    ground_pos = []
    for i in range(N):
        h = get_pos(b[i])[-1]
        ground_pos.append(h)
    all_ground_pos.append(ground_pos)

hs = np.array(all_ground_pos)


from scipy import optimize, signal

med_positions = np.zeros((6, 3))
for ix_leg in range(len(legs)):
    ii = signal.find_peaks(-hs[ix_leg, :, 2], distance=30)[0]
    med_positions[ix_leg] = np.median(hs[ix_leg][ii], axis=0)

def get_heights(theta, phi, offset, poses):
    return np.tan(theta) * (poses[:, 0] - offset[0]) + \
        np.tan(phi) * (poses[:, 1] - offset[1]) + \
        offset[2]

def error(params, poses):
    theta = params[0]
    phi = params[1]
    offset = params[2:5]
    heights = get_heights(theta, phi, offset, poses)
    error = heights - poses[:, 2]
    return error

params = np.array([0, 0, 0, 0, 0])

opt = optimize.least_squares(error, params, args=(med_positions,))

theta, phi, offset = opt.x[0], opt.x[1], opt.x[2:5]

offset = np.copy(offset)
offset[2] += 0.2

ground    = GroundModel(offset=offset,
                        phi=np.rad2deg(phi),
...
plt.plot(pose_3d[:, :, -1, -1] - thresholds)
# plt.plot(thresholds, linestyle='dashed')
plt.draw()
plt.show(block=False)


thresholds = np.zeros(pose_3d.shape[:2])
for i in range(thresholds.shape[0]):
    for j in range(thresholds.shape[1]):
        thresholds[i, j] = ground.get_height(pose_3d[i, j, -1 ,0], pose_3d[i, j, -1, 1])
