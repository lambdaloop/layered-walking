...
# plt.clf()
# plt.subplot(211)
# plt.plot(bout['contexts'])
# plt.subplot(212)
# plt.plot(signal.resample(bout['contexts'], len(bout['contexts']) * ctrlSpeedRatio, axis=0))
# plt.draw()
...
# fictrac_speeds = [6, 8, 10, 12, 14, 16]
...
full_conditions = [
    ...
    for [f_speed, f_rot, f_side] in video_speeds
...
for ix_cond, cond in enumerate(tqdm(conditions, ncols=70)):
    ...
    bout = wd.get_bout(context, offset=offset)
    # contexts = bout['contexts']
    # contexts = [context for _ in range(numSimSteps)]
    contexts = signal.resample(bout['contexts'],
                               len(bout['contexts']) * ctrlSpeedRatio,
                               axis=0)
    contexts = contexts[:numSimSteps]
