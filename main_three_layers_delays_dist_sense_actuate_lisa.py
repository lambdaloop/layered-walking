...
if len(sys.argv) > 1:
    ...
    outfilename = "delays_stats_subang_v2_sense_actuate_poisson"
...
# filename = '/home/lili/data/tuthill/models/models_sls/data_subang_5.pickle'
...
# sense_delays = np.arange(0, 0.020, 0.002)

# act_delays = np.arange(0, 0.061, 0.0025)
# sense_delays = np.arange(0, 0.061, 0.0025)
...
act_sense_delays = np.array(act_sense_delays) / 1000.0
...
full_conditions = [
    ...
    for (act_delay, sense_delay) in act_sense_delays
...
outpath = outname.format(start_index)
...
actual_delays = list(set([(x['actDelay'], x['senseDelay']) for x in conditions]))

CD_dict = dict()

for actDelay, senseDelay in tqdm(actual_delays, ncols=70, desc="making controllers"):
    dAct = int(actDelay / Ts * ctrlSpeedRatio)
    dSense = int(senseDelay / Ts * ctrlSpeedRatio)
...
for ix_cond, cond in enumerate(tqdm(conditions, ncols=70)):
    ...
    CD = CD_dict[(dSense, dAct)]
    if any([c is None for c in CD]):
        continue # don't have controller, skip
