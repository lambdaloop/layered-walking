...
if len(sys.argv) < 2:
    print("pass number of principal components as first argument please!")
    exit()

n_pca_components = int(sys.argv[1])
print("number of PCA components: {}".format(n_pca_components))
...
subixs = np.random.choice(pcs.shape[0], size=20000, replace=False)
kde = gaussian_kde(pcs[subixs,:n_pca_components].T)
...
pcs = np.full((angles_sc_flat.shape[0], n_pca_components), np.nan)
...
for i in trange(0, len(pcs), step, ncols=70):
  ...
  pdfs_flat[i:i+step][check] = kde.logpdf(pcs[i:i+step, :n_pca_components][check].T)
...
np.savez_compressed('sense_actuation_angle_pdfs_pc{}.npz'.format(n_pca_components), pdfs=pdfs_shaped)
...
#         plt.savefig('plots/act_delays_trace_pc{}_{}_{}.pdf'.format(n_pca_components, angname, dist),
#                     bbox_inches = "tight")
...
for dist_name, dist in [('continuous', DistType.POISSON_GAUSSIAN),
    ...
    for xaxis in ['speed', 'max_velocity']:
        ...
        for (name, key) in to_plot:
            ...
            plt.savefig('plots/actsensedelay_logpdf_pc{}_{}_{}.pdf'.format(n_pca_components, dist_name, key),
                        bbox_inches = "tight")
